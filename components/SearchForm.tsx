...
export default function SearchForm() {
  useEffect(() => {
    ...
    const up = (el: HTMLInputElement | null) =>
    ...
    trip?.addEventListener("click", (e) => {
      ...
      if (btn.dataset.trip === "oneway") { ret.value = ""; ret.disabled = true; }
      ...
      <div className="rail">
        <form className="bg-white border border-br rounded-xl shadow-xl p-4" action="/flights/results" method="get">
          ...
          <div className="grid gap-3 grid-cols-[1.2fr_1.2fr_1fr_1fr_auto] items-end">
            <div>
              <label htmlFor="origin" className="text-xs text-muted">From</label>
              <input id="origin" name="origin" className="input" placeholder="LON" required />
            </div>
            <div>
              <label htmlFor="destination" className="text-xs text-muted">To</label>
              <input id="destination" name="destination" className="input" placeholder="BKK" required />
            </div>
            <div>
              <label htmlFor="departure_at" className="text-xs text-muted">Depart</label>
              <input id="departure_at" name="departure_at" type="date" className="input" required />
            </div>
            <div id="return-block">
              <label htmlFor="return_at" className="text-xs text-muted">Return</label>
              <input id="return_at" name="return_at" type="date" className="input" />
            </div>
          ...
          <div className="grid gap-3 grid-cols-[1fr_1fr_1fr_1fr_auto] mt-3 items-end">
            ...
            <div><label className="text-xs text-muted" htmlFor="currency">Currency</label>
              <select id="currency" name="currency" className="input">
                <option value="GBP">GBP</option><option value="USD">USD</option><option value="EUR">EUR</option><option value="THB">THB</option>
              </select>
            </div>
